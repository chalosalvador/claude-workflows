...
- **The gate** is [§ The gate](CONTRIBUTING.md#the-gate); CI runs it as the `guards` job.
  Stage the paths you changed before running it, because three of the guards read the
  index.
...
- **The installed plugin is a cached copy.** Your edits reach no session until the version
  is bumped and updated, and a `git checkout` changes nothing it serves. Test skills with
  `claude --plugin-dir ./plugins/gh-issue-flow`, and restart after `claude plugin update`:
