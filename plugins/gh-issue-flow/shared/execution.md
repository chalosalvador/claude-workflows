...
`--plugin-dir` the session was started with, read from its process arguments since the
session cannot otherwise tell, or else the `path` of `gh-issue-flow` in a fresh session's
init event; the start prompt's `Plugin docs:` line carries the command
