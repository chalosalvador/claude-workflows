...
honest state is "we do not know how to verify a secret here yet" — say that, do not
provision one.
...
- **Assert on public surfaces**, never framework internals, unless the test is about
  internals. The public surface — the OpenAPI document, the exported routes list — is
  also what an external post-deploy check reads, so tests and runbook cannot disagree
  about what "mounted" means.
