...
- **Distinguish the two comment shapes by body, never by count.** The ack and
  real-review patterns are bot-specific: read them from the repo's `repo.md` § Review
  ...
  comments for a human to commit. You do not edit `repo.md`. A loop that cannot tell
