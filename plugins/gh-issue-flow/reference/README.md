...
| [`secrets-and-ci.md`](secrets-and-ci.md) | Provisioning a secret or environment variable — the rules true on every platform; the commands for *this* platform are in the repo's deploy-target doc. Also green-local/red-CI. |

**Nothing stack-specific is in this directory.** How a secret is verified on this
platform, what merging deploys, which bot reviews PRs and what its comments look like,
which invariants a reviewer must check — that lives in each repo's
`.claude/workflow/deploy-targets/<name>.md`, generated by `setup` and owned by the repo. See
