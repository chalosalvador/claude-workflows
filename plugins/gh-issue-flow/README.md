...
### Stack docs — in your repo, not here

`.claude/workflow/stacks/<name>.md`, one file per deploy target, named in
`workflow.json` → `stacks`, generated by `setup` from what it probed and filled in by the
people who run the platform. Skills read the section headers — Identity, Deploy, Secrets
and env, Infra and migrations, Review bot, Reviewer invariants, Traps — at the moment
each matters. What an `UNVERIFIED` section means to a reader, and who may write to a
stack doc, are rules [`shared/config.md`](shared/config.md) § Stack docs owns.
