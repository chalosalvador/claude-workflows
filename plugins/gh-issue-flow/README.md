...
`.claude/workflow/stacks/<name>.md`, one file per deploy target, generated by `setup`
from what it probed and filled in by the people who run the platform. Skills read the
section headers — Deploy, Secrets and env, Infra and migrations, Review bot, Reviewer
invariants, Traps — at the moment each matters. A section marked `UNVERIFIED` is an
