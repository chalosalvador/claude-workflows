...
Setup also writes **deploy-target docs** — `.claude/workflow/deploy-targets/<name>.md`, one per
place code gets deployed, not per technology — and one **`repo.md`** for the review bot,
the reviewer invariants and the traps, from what it probed, marking everything it could
not probe `UNVERIFIED` for a human to fill in. The plugin's own reference docs are stack-neutral;
...
gained since your `workflow.json` was written and generates any deploy-target doc that new
...
config resolution), [`shared/board.md`](plugins/gh-issue-flow/shared/board.md) (board
resolution and queries, loaded only by the board skills) and
[`shared/execution.md`](plugins/gh-issue-flow/shared/execution.md) (branch, validate,
review, babysit, board, deploy — facts, not policy).

**Deploy-target docs and `repo.md`** — per repo, in `.claude/workflow/`, generated by `setup` and owned
